...
const MovieCard = ({ movieItem }) => {
  ...
  return (
    <>
      ...
      <motion.div initial={{ opacity: 0, top: -50, position: "relative" }} animate={{ opacity: 1, top: 0 }} transition={{ duration: 0.3, delay: 0.2 }} onTapStart={() => handleMovieContent(movieItem.id, movieItem.media_type || "movie")} className="relative px-2 pt-2 m-5 duration-300 bg-white rounded-lg cursor-pointer hover:scale-105 card hover:bg-dark hover:text-light card-shadow w-80">
        ...
        <div className="w-full overflow-hidden max-h-[27rem]">{!movieItem.poster_path ? <LazyLoadImage src={`https://i.pinimg.com/originals/51/9c/18/519c18a68160ffb6d5aa5d92cd1e3d59.jpg`} alt="" effect="blur" /> : <LazyLoadImage className="hover-scale-duration" src={`${IMG_API}${movieItem.poster_path}`} alt="" effect="blur" />}</div>
